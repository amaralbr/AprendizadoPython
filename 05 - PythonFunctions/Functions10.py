# Jogo de Craps. Faça um programa de implemente um jogo de Craps. O jogador lança um par de dados, obtendo um valor entre 2 e 12.
# Se, na primeira jogada, você tirar 7 ou 11, você um "natural" e ganhou. Se você tirar 2, 3 ou 12 na primeira jogada, isto é chamado de
# "craps" e você perdeu. Se, na primeira jogada, você fez um 4, 5, 6, 8, 9 ou 10,este é seu "Ponto". Seu objetivo agora é continuar jogando os
# dados até tirar este número novamente. Você perde, no entanto, se tirar um 7 antes de tirar este Ponto novamente.


########## P R O G R A M A ##########
import random
def dados():
    dado1 = random.randint(1, 6)
    dado2 = random.randint(1, 6)
    return dado1 + dado2
cont = 1
pontos = 0
while True:
    input('------- Lance os Dados -------')
    result = dados()
    if cont == 1:
        if result == 7 or result == 11:
            print(f'>>{result}<<-- Você ganhou! --')
            break
        elif result == 2 or result == 3 or result == 12:
            print(f'>>{result}<< -Você perdeu!- ')
            break
        else:
            pontos += result
            print(f'>>{result}<< Soma de pontos: {pontos}')
            cont += 1
    else:
        if result == 7:
            print(f'>>{result}<< -Você perdeu!- ')
            break
        else:
            pontos += result
            print(f'>>{result}<< Soma de pontos: {pontos}')
